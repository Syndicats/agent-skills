...
This is a **universal CLI tool** (`skills` / `agent-skills`) for managing AI agent skills across 45+ platforms (Cursor, Claude Code, Copilot, etc.). Written in TypeScript (ES2022, NodeNext modules).

### Source Layout

- **`src/cli/`** - CLI layer built on `commander`. Entry point is `index.ts`, which registers ~40 commands from `commands/*.ts`. Agent platform definitions live in `agents.ts` (maps agent names to their project/global skill directories).
- **`src/core/`** - Business logic. Key modules:
  ...
  - `marketplace.ts` - Marketplace source management and skill fetching
  - `skillsdb.ts` - Supabase-backed skills database API (primary data source)
  ...
  - `conflict-detector.ts`, `context-budget.ts`, `differ.ts`, `composer.ts`, `splitter.ts`, `skill-tester.ts` - Power tool engines (v1.1.4+)
...
- **`skills/`** - Example/bundled skills (excluded from compilation).
...
- **Dual data sources**: Skills come from either the Supabase database API (`skillsdb.ts`) with GitHub as fallback, or from direct Git/npm sources.
